...
export function ConversationItem({
  ...
  const timeText = conversation.updatedAt ? formatRelativeTime(conversation.updatedAt) : "2 hours ago";
  ...
  if (isCollapsed) {
    return (
      <button
        ...
        className={cn(
          "group relative flex h-10 w-full items-center justify-center rounded-xl transition-[border-color,background-color,color] duration-150 cursor-pointer",
          isActive || isHovered ? "active-row-glow text-white" : "text-zinc-400 hover:text-white hover:bg-white/5",
        )}
      >
        <MessageSquare
          className={cn(
    ...
    >
      {isEditing ? (
        ...
        <div className="flex items-center justify-between gap-2 min-w-0">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <span
              className={cn(
                ...
                isActive || isHovered ? "font-semibold text-white" : "font-normal text-zinc-300 group-hover:text-white",
              )}
            >
              {conversation.title}
            </span>
          ...
          <span className="text-[10px] text-zinc-500 shrink-0 font-mono">{timeText}</span>
        </div>
      )}
    </div>
  );
