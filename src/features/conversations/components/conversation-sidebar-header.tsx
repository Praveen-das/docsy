...
export function ConversationSidebarHeader({ isCollapsed, onToggleCollapse, onClose }: ConversationSidebarHeaderProps) {
  return (
    <>
      {/* Brand Header */}
      <div className="flex h-16 shrink-0 items-center border-b border-zinc-200 px-3 relative overflow-hidden dark:border-white/5">
        {isCollapsed ? (
          ...
          <div className="flex items-center justify-between w-full min-w-0">
            ...
            <button
              ...
              className="hidden lg:flex rounded-md p-2 text-zinc-500 hover:text-zinc-900 hover:bg-zinc-200/60 transition-colors cursor-pointer active:scale-95 shrink-0 dark:text-zinc-400 dark:hover:text-zinc-100 dark:hover:bg-white/5"
      ...
      <div className="px-3 py-2 border-b border-zinc-200/70 dark:border-white/5">
        <Link
          href="/documents"
          onClick={onClose}
          title={isCollapsed ? "All Documents" : undefined}
          className="group relative flex h-9 items-center rounded-md text-xs font-medium transition-colors select-none tracking-[-0.01em] p-0 w-full overflow-hidden text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-300"
        >
          ...
          {!isCollapsed && <span className="truncate pr-3 font-medium text-xs">All Documents</span>}
        </Link>
      </div>
    </>
  );
